...
for index, row in data.iterrows():
    ...
    elif data['Commentary_IDs'][index] in ['Nothing','nothing']:
        e.append((data['Title'][index], data['Title'][index]))
    if len(e)>0:        
        # print(e)
        G.add_edges_from(e)
...
pos = nx.spring_layout(G, scale=2, k=10*1/np.sqrt(len(G.nodes())), iterations=10)
...
nx.draw(G, pos=pos, node_color='#A0CBE2',width=0.2, line_color='grey', ax=ax)
